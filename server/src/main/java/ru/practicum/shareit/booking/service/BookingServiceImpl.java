...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingDto approveBooking(int bookingId, boolean approved, int userId) {
        ...
        if (BookingState.APPROVED.equals(approvingBooking.getStatus())) {
            throw new WrongBookingRequestException("Booking is already approved.");
        }
        if (approved) {
            approvingBooking.setStatus(BookingState.APPROVED);
        } else {
            approvingBooking.setStatus(BookingState.REJECTED);
        }
    ...
    public List<BookingDto> getBookingsOfUser(String state, int userId, Pageable pageable) {
        ...
        switch (state) {
            ...
            case "FUTURE":
                ...
                break;
            case "WAITING":
                bookings = bookingRepository.findAllByBookerIdAndStatusEqualsOrderByStartDesc(userId, BookingState.WAITING, pageable);
                break;
            case "REJECTED":
                bookings = bookingRepository.findAllByBookerIdAndStatusEqualsOrderByStartDesc(userId, BookingState.REJECTED, pageable);
                break;
            case "ALL":
    ...
    public List<BookingDto> getBookingsOfOwner(String state, int userId, Pageable pageable) {
        ...
        switch (state) {
            ...
            case "FUTURE":
                ...
                break;
            case "WAITING":
                bookings = bookingRepository.findAllByOwnerAndStatusAndOrderByStartDesc(userId, BookingState.WAITING, pageable);
                break;
            case "REJECTED":
                bookings = bookingRepository.findAllByOwnerAndStatusAndOrderByStartDesc(userId, BookingState.REJECTED, pageable);
                break;
            case "ALL":
